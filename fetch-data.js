...
function getWeekCols(rows) {
  ...
  // raw:true returns Excel serial numbers for dates
  const cols = [];
...
function parseGIM(rows) {
  ...
  for (let ri = 6; ri < rows.length; ri++) {
    ...
    if (weeks.length) brokers.push({ name, group, weeks });
  }
  return brokers;
}
...
function parseIA(rows) {
  ...
  const brokers = [];
  for (let ri = 8; ri < 111; ri++) {  // rows 111-149 are GIM section, excluded
    ...
    // YTD summary columns
    const inv_ytd      = safeNum(rows[ri][276]); // Issued Cases Gross
    const trails_ytd   = safeNum(rows[ri][277]); // Trails & Fees
    const w_indem_ytd  = safeNum(rows[ri][280]); // Written Indemnified
    const w_non_ytd    = safeNum(rows[ri][281]); // Written Non-Indemnified
    if (weeks.length || inv_ytd || trails_ytd) brokers.push({ name, team, inv_ytd, trails_ytd, w_indem_ytd, w_non_ytd, weeks });
  }

  // Remove GIM brokers (belong in GIM dashboard) and catch-all Others rows
  return brokers.filter(b => b.team !== "Other");
}
