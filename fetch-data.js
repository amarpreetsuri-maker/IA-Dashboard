...
function parseIA(rows) {
  ...
  for (let ri = 8; ri < 111; ri++) {  // rows 111-149 are GIM section, excluded
    ...
    // YTD summary from col 276 (Issued Cases Gross) and col 279 (Net Revenue)
    const inv_ytd = safeNum(rows[ri][276]);
    const rev_ytd = safeNum(rows[ri][279]);
    if (weeks.length || inv_ytd || rev_ytd) brokers.push({ name, team, inv_ytd, rev_ytd, weeks });
  }
